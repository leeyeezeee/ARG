...
import sys
import datetime
from finetune_svamp import setup_environment
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate model on SVAMP")
    parser.add_argument('--model_path', type=str,
                        default='output/xxxx',
                        help="Path to trained model directory")
    parser.add_argument('--dataset_path', type=str,
                        default='../../local_datasets/SVAMP/SVAMP.json',
    ...
    parser.add_argument('--output_file', type=str,
                        default='svamp_eval_results.jsonl',
...
async def main(ef=True):
    ...
    full_dataset_raw = JSONReader.parse_file(args.dataset_path)  # SVAMP 是 JSON 数组
    full_dataset = svamp_data_process(full_dataset_raw)

    if not os.path.exists(args.task_split_path):
        raise FileNotFoundError(f"Task split file '{args.task_split_path}' not found. Run cold_start_svamp.py first.")
    ...
    print(f"Loaded {len(dataset)} SVAMP test samples for evaluation.")
