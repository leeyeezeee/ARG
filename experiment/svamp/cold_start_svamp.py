...
OUTPUT_DIR = "../ColdStartData_svamp"
TASK_SPLIT_FILE = "./task_split_svamp.json"
BASE_RATE = 0.4


def parse_args():
    parser = argparse.ArgumentParser(description="Generate cold-start data for SVAMP")
    parser.add_argument('--dataset_json', type=str, default="../../local_datasets/SVAMP/SVAMP.json",
                        help="Path to SVAMP JSON dataset")
...
def get_unique_complex_configs_svamp():
    configs = set()
...
def write_to_csv_svamp(data):
    filename = "records_svamp.csv"
...
async def evaluate_and_save_svamp(
        graph: Graph,
        dataset,
        args,
        current_mode: str,
...
async def main():
    args = parse_args()
    raw_dataset = JSONReader.parse_file(args.dataset_json)  # SVAMP 是 JSON 数组
    ...
    configs = get_unique_complex_configs_svamp()
    print(f"Generating SVAMP cold-start data for {len(configs)} configurations...")
    ...
    print("All SVAMP cold-start data generation complete.")
