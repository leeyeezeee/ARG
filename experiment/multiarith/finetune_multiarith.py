...
FINETUNE_DATA_DIR = "../FinetuneData_multiarith"
COLD_START_DIR = "../ColdStartData_multiarith"
TASK_SPLIT_FILE = "./task_split_multiarith.json"


def parse_cli_args():
    parser = argparse.ArgumentParser(description="Full pipeline for MultiArith: pretrain, generate data, finetune")
    ...
    parser.add_argument('--model_output_dir', type=str, default='output/multiarith_finetuned_model',
...
def run_pretraining(args):
    print("\n" + "=" * 20 + " Stage 1: Pretraining MultiArith Model " + "=" * 20)
    pre = Args().update_args()
    pre.dataset = 'multiarith'
...
async def generate_pruned_data(args, ds, mp, od):
    print("\n" + "=" * 20 + " Stage 2a: MultiArith Pruned Data " + "=" * 20)
    model = load_model(mp)
    model.eval()
...
async def generate_simple_data(args, ds, od):
    print("\n" + "=" * 20 + " Stage 2b: MultiArith Simple Data " + "=" * 20)
    ...
    for mode, num in configs:
        ...
        await evaluate_and_save_multiarith_simple(g, ds, args, mode, num, od)
    print("Stage 2b done")


def generate_replay_data(args, od):
    print("\n" + "=" * 20 + " Stage 2c: MultiArith Replay Data " + "=" * 20)
...
async def evaluate_and_save_multiarith_simple(graph, ds, args, mode, num, od):
    total_solved = 0
    ...
    for i_batch in tqdm(range(num_batches), desc=f"Processing {mode}-{num}"):
        ...
        for i, result in enumerate(results):
            ...
            if is_solved:
                total_solved += 1
                name = "_".join(map(str, ['multiarith', metadata['record_idx'], mode, num, 'True']))
...
def run_finetuning(model_path, config_path, finetune_data_dir, args):
    print("\n" + "=" * 20 + " Stage 3: MultiArith Finetuning " + "=" * 20)
    finetune_args = Args()
    ...
    finetune_args.dataset = 'multiarith'
    ...
    print("=" * 20 + " Stage 3: MultiArith Finetuning Complete " + "=" * 20)


def find_latest_model_dir(bd='output', pf='multiarith_finetuned_model'):
...
async def main():
    ...
    if args.pretrain:
        ...
        print(f"Starting new MultiArith run, outputs to: {args.model_output_dir}")
    ...
    if not os.path.exists(TASK_SPLIT_FILE):
        raise FileNotFoundError(f"Task split file '{TASK_SPLIT_FILE}' not found. Please run cold_start_multiarith.py first.")
    with open(TASK_SPLIT_FILE, 'r') as f:
        task_split = json.load(f)
    
    full_dataset = multiarith_data_process(JSONReader.parse_file("../../local_datasets/MultiArith/MultiArith.json"))
    finetune_dataset_subset = [full_dataset[i] for i in task_split['finetune_tasks_indices']]
    print(f"\nLoaded {len(finetune_dataset_subset)} tasks (from training set) for MultiArith finetune data generation.")
    ...
    print(f"\nMultiArith pipeline complete! Final efficient model at: {os.path.join(args.model_output_dir)}")
