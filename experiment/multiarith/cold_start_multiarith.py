...
OUTPUT_DIR = "../ColdStartData_multiarith"
TASK_SPLIT_FILE = "./task_split_multiarith.json"
BASE_RATE = 0.4


def parse_args():
    parser = argparse.ArgumentParser(description="Generate cold-start data for MultiArith")
    parser.add_argument('--dataset_json', type=str, default="../../local_datasets/MultiArith/MultiArith.json",
    ...
    parser.add_argument('--domain', type=str, default="gsm8k",
                        help="Task domain name (reuse gsm8k for prompts)")
    return parser.parse_args()


def get_unique_complex_configs_multiarith():
    configs = set()
...
def write_to_csv_multiarith(data):
    filename = "records_multiarith.csv"
...
async def evaluate_and_save_multiarith(
        graph: Graph,
        dataset,
        args,
        current_mode: str,
    ...
    for i_batch in tqdm(range(num_batches), desc=f"Processing {current_mode}-{current_agent_num}"):
        ...
        for i, result in enumerate(results):
            ...
            write_to_csv_multiarith(batch_data)
...
async def main():
    args = parse_args()
    raw_dataset = JSONReader.parse_file(args.dataset_json)  # MultiArith 是 JSON 数组
    ...
    configs = get_unique_complex_configs_multiarith()
    print(f"Generating MultiArith cold-start data for {len(configs)} configurations...")
    ...
    for mode, agent_num in configs:
        ...
        await evaluate_and_save_multiarith(
            graph=graph,
    ...
    print("All MultiArith cold-start data generation complete.")
